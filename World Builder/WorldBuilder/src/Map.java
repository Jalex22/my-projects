...
public class Map
{
	...
	private int pixelWidth = blockWidth * 16;
	private int pixelHeight = blockHeight * 16;
	...
	public boolean checkCollision(Rectangle rect, int layer, int xZoom, int yZoom) {
		...
		//check tile in 4radius from player
		//overlaps a little in collision when radius<4
		...
		for(int x = topLeftX; x < bottomRightX; x++)
		{
			for(int y = topLeftY; y < bottomRightY; y++) 
			{
				...
				if(tile != null) 
				{
					...
					if(collisionType == 0) //collision from any side
					{
	...
	public void setTile(int layer, int tileX, int tileY, int tileID)
	{
		...
		else 
		{
			...
			{
				int inc = blockX * -1;
				newMinX = blockStartX - blockWidth*inc;
				newLengthX = newLengthX + inc;
			...
			{
				int inc = blockY * -1;
				newMinY = blockStartY - blockHeight*inc;
				newLengthY = newLengthY + inc;
	...
	public void render(Render renderer, GameObject[] objects, int xZoom, int yZoom)
	{
		int tileWidth = 16 * xZoom;
		int tileHeight = 16 * yZoom;
